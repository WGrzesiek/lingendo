...
import {algorithms, REVIEW_SCHEDULE_LABELS, reviewSchedules} from "@/types/learning";
...
export const CourseSettings = ({ enrollmentId }: CourseSettingsProps) => {
  ...
  const { mutate: mutateLimit, isPending: isLimitPending } = useUpdateFlashcardsPerSession();
  ...
  const [selectedAlgorithm, setSelectedAlgorithm] = useState<(typeof algorithms)[number] | null>(null);

  const {
    mutate: mutateAlgorithm,
    isPending: isAlgorithmPending,
  ...
  const [openReviewScheduleDialog, setOpenReviewScheduleDialog] = useState(false);
  const [reviewSchedule, setReviewSchedule] = useState<reviewSchedules>("AUTO");
  const [selectedReviewSchedule, setSelectedReviewSchedule] = useState<reviewSchedules>("AUTO");
  const {mutate: mutateReviewSchedule, isPending: isReviewSchedulePending} = useUpdateReviewSchedule();

  const handleReviewScheduleChange = (schedule: reviewSchedules) => {
    setSelectedReviewSchedule(schedule);
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                krok w nowym algorytmie oraz zmianę sposobu planowania powtórek
                w tym kursie.
              </span>
      ...
      <div className="pt-4 border-t space-y-4">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <DialogFooter>
              <Button variant="ghost" onClick={() => setOpen(false)} disabled={isLimitPending}>
                Anuluj
              </Button>
