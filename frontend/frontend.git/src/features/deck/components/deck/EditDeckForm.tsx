...
import { ArrowRightLeft, Globe, Settings2, Calendar, Save } from "lucide-react";

import { useUpdateDeckDetails } from "../../hooks/mutation/useUpdateDeckDetails";
...
import { VISIBILITIES, VisibilityValue, Visibility } from "@/types/learning";
import type { DeckDetailsDto } from "@/features/deck/types";
...
export const EditDeckForm = ({ deckId, initialData }: EditDeckFormProps) => {
  const router = useRouter();
  const { mutate: updateDeck, isPending } = useUpdateDeckDetails();
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      learnAlgorithm: initialData.learnAlgorithm || "GRZESIEK_ALGORITHM",
      howManyFlashcardsForOneSession:
        Number(initialData.howManyFlashcardsForOneSession) || 10,
      languageFrom: initialData.languageFrom || "ENGLISH",
      languageTo: initialData.languageTo || "POLISH",
      owner: initialData.owner || "I",
  ...
  const onSubmit = (values: FormValues) => {
    const payload: DeckDetailsDto = {
      ...initialData,
      ...values,
      category: values.category as DeckCategory,
      visibility: values.visibility as Visibility,
      difficulty: values.difficulty,
      howManyFlashcardsForOneSession: Number(
        values.howManyFlashcardsForOneSession
      ),
    };
    ...
    <Card className="max-w-3xl mx-auto shadow-lg border-muted/60">
      <CardHeader className="bg-muted/20 pb-8 border-b">
        <div className="flex items-center gap-3 mb-2">
          ...
          <CardTitle className="text-2xl">Edycja Kursu</CardTitle>
        </div>
        <CardDescription className="text-base">
          Zaktualizuj informacje o swoim kursie. Zmień nazwę, opis lub
          ustawienia.
        </CardDescription>
      </CardHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-8 pt-8">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription className="text-xs">
                      Język, w którym będą prezentowane słówka.
                    </FormDescription>
                  </FormItem>
                )}
              />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription className="text-xs">
                      Język, w którym będą odpowiedzi i tłumaczenia.
                    </FormDescription>
                  </FormItem>
                )}
              />
            ...
            <div className="relative py-4">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-card px-2 text-muted-foreground flex items-center gap-2">
                  <Settings2 className="w-4 h-4" /> Zaawansowane opcje
                </span>
              </div>
            </div>
            ...
            <div className="bg-secondary/20 p-6 rounded-xl space-y-6 border border-secondary/40">
              <div className="grid sm:grid-cols-2 gap-6">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Algorytm nauki</FormLabel>
                      ...
                      <FormDescription>
                        Wybierz metodę, która najlepiej pasuje do Twojego stylu
                        nauki.
                      </FormDescription>
                    </FormItem>
                  )}
