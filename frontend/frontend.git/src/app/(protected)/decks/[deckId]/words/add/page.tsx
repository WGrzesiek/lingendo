...
import { CommunityWordsSelector } from "@/features/deck/components/words/CommunityWordsSelector";
import { WordToAdd, emptyWord } from "@/features/deck/types/word.types";

/**
 * Zmockowane dane kursu
 */
const mockDeck = {
  deckId: "deck-1",
  deckName: "Angielski - Czasowniki nieregularne",
  currentWordCount: 145,
};

/**
 * Strona dodawania słówek do kursu
 * Obsługuje dodawanie pojedyncze (formularz) i batchowe (JSON)
 */
const AddWordsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "form" | "json" | "community" | "addToCommunity"
  >("form");
  ...
  const handleCommunitySelect = async (wordIds: string[]) => {
    ...
    try {
      ...
      console.log("Dodawanie słówek ze społeczności:", wordIds);

      setSuccessMessage(
        `Pomyślnie dodano ${wordIds.length} słówek ze społeczności do kursu "${mockDeck.deckName}"!`
      );
      ...
      alert("Wystąpił błąd podczas dodawania słówek ze społeczności");
    } finally {
  ...
  const handleSubmitToCommunity = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Pomyślnie dodano ${words.length} słówek do biblioteki społeczności!`
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Zapisywanie słówek do kursu:", deckId, words);

      setSuccessMessage(
        `Pomyślnie dodano ${words.length} słówek do kursu "${mockDeck.deckName}"!`
      );
      ...
      alert("Wystąpił błąd podczas zapisywania słówek");
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto p-6 lg:p-8 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-muted-foreground mt-1">
              Kurs: <span className="font-medium">{mockDeck.deckName}</span> (
              {mockDeck.currentWordCount} słówek)
            </p>
          </div>
        </div>
        ...
        <Tabs
          ...
          onValueChange={(value) =>
            setActiveTab(
              value as "form" | "json" | "community" | "addToCommunity"
            )
          }
        >
          <TabsList className="grid w-full grid-cols-2 lg:grid-cols-4 max-w-4xl">
            ...
            <TabsTrigger value="addToCommunity">
              <Upload className="w-4 h-4 mr-2" />
              Dodaj do społeczności
            </TabsTrigger>
          </TabsList>
          ...
          {/* Słówka ze społeczności */}
          <TabsContent value="community" className="mt-6">
            <CommunityWordsSelector
              onSelect={handleCommunitySelect}
              isAdding={isSubmitting}
            />
          </TabsContent>

          {/* Dodaj słówka do społeczności */}
          <TabsContent value="addToCommunity" className="mt-6 space-y-6">
            <Alert>
              <Upload className="h-4 w-4" />
              <AlertDescription>
                Dodaj słówka do biblioteki społeczności, aby inni użytkownicy
                mogli z nich korzystać. Twoje słówka będą dostępne dla
                wszystkich!
              </AlertDescription>
            </Alert>
