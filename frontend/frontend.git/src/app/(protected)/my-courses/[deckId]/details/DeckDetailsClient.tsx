...
console.log(deckId);
    ...
    // Pobieranie fiszek z infinite scroll
    const {
        data: flashcardsData,
...
// Połącz wszystkie strony fiszek
    const allFlashcards = useMemo(() => {
        return flashcardsData?.pages.flatMap((page) => page.content) ?? [];
    }, [flashcardsData]);
    const totalElements = flashcardsData?.pages.flatMap((page) => page.totalElements);
// Filtrowanie słówek po zapytaniu
    ...
    // Przekształć dane API do formatu używanego w komponentach
    ...
    const stats: DeckStats | null =
        statsData && statsData[deckId]
            ? {
                ...
                activeStudents: statsData[deckId].totalStudents - (statsData[deckId].completedStudents || 0),
                totalViews: 1000, //NOTE dorobic event w stats
            }
            : null;
    ...
    return (
        <div className="min-h-screen bg-background">
            <div className="container mx-auto p-6 lg:p-8 space-y-6">
                ...
                {deck.isOwner && stats && <DeckDetailsStats stats={stats} />}
