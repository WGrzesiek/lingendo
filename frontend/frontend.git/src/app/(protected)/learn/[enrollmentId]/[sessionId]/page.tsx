...
const LearningSessionPage = ({ params }: { params: { enrollmentId: string, sessionId: string } }) => {
  const router = useRouter();
  const sessionId = params.sessionId;
  const enrollmentId = params.enrollmentId;
  const {data: sessionHeader, isLoading: isSessionHeaderLoading, isError: isSessionHeaderError} = useCourseHeader(enrollmentId);
  const {data: sessionSettings, isLoading: isSessionSettingsLoading, isError: isSessionSettingsError} = useCourseSettings(enrollmentId)
  const {data: sessionProgress, isLoading: isSessionProgressLoading, isError: isSessionProgressError} = useCourseProgress(enrollmentId);
  const {data, isLoading, isError, error, refetch,} = useNextFlashcardRecommendation(sessionId);
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-accent/5 to-background">
        ...
        {selectedAlgorithm.name}
                ...
                <div className="text-base font-medium text-muted-foreground">
                  ...
                  {
                    sessionProgress.sessions.find(s => s.status === "IN_PROGRESS")
                        ?.sessionNumber
                  }
                </div>
              </div>
