...
-- 1) DECK:
ALTER TABLE deck ADD COLUMN owner varchar(255) NOT NULL DEFAULT 'USER';
ALTER TABLE deck ADD CONSTRAINT deck_owner_check CHECK (owner IN ('USER', 'TEACHER', 'FRIEND', 'COMMUNITY'));
...
ALTER TABLE session ADD CONSTRAINT session_status_check CHECK (status IN ('IN_PROGRESS', 'COMPLETED', 'ABANDONED'));
