pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "${REGISTRY_HOST}/learnwords/api-gateway"
    }
    parameters {
    ...
    stages {
        stage('Compose Down') {
            ...
            steps {
                sh "docker compose -f docker-compose.api-gateway.yml down"
            }
        }
        stage('Compose Restart') {
            when { expression { params.ACTION == 'restart' } }
            steps {
                sh "docker compose -f docker-compose.api-gateway.yml restart"
            }
        }
        ...
        stage('Checkout') {
            ...
            steps {
                checkout scmGit(
                    ...
                    userRemoteConfigs: [[
                        url: 'git@bitbucket.org:grzegorz5/api-gateway.git',
        ...
        stage('Uruchom Docker Compose') {
            ...
            steps {
                sh """
                    ...
                    IMAGE_NAME=${IMAGE_NAME} IMAGE_TAG=${IMAGE_TAG} SPRING_PROFILES_ACTIVE=${SPRING_PROFILES_ACTIVE} docker compose -f docker-compose.api-gateway.yml up -d
                """
            }
        }
    }
